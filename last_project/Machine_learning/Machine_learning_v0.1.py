...
scaler = MinMaxScaler()
scaled = scaler.fit_transform(df_learn[all_features])



# 특징 변수(X)와 타겟 변수(y) 분리
df_learn = pd.DataFrame(scaled, columns = df_learn[all_features].columns)
...
random_search = RandomizedSearchCV(model, param_distributions=param_dist, n_iter=10, cv=5, scoring='neg_mean_squared_error', random_state=42, n_jobs=-1, error_score='raise')
...
rand_search_lgb = RandomizedSearchCV(lgb_clf, param_distributions=param_dist_lgb, n_iter=10, cv=5, scoring='accuracy', random_state=1)
